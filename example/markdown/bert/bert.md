...
2.1Unsupervised Feature-based Approaches
...
Masked Sentence AMasked Sentence B
...
Model ArchitectureBERT’s model architec-
...
BERT_{BASE} (L=12, H=768, A=12, Total Param-
eters=110M) and BERT_{LARGE} (L=24, H=1024,
...
<!-- page:4 -->
Input/Output RepresentationsTo make BERT
...
Task #1: Masked LMIntuitively, it is reason-
...
<!-- page:5 -->
Input[CLS]mydogiscute[SEP]helikesplay##ing[SEP]

Token
EEEEEEEEEEE
Embeddings[CLS]mydogiscute[SEP]helikesplay##ing[SEP]

Segment
EmbeddingsEAEAEAEAEAEAEBEBEBEBEB

Position
EmbeddingsE0E1E2E3E4E5E6E7E8E9E10
...
den vector C ∈R^{H} corresponding to the first
...
R^{K}^{×H}, where K is the number of labels. We com-
pute a standard classification loss with C and W,
i.e., log(softmax(CW ^{T} )).

^{7}For example, the BERT SQuAD model can be trained in
...
392k363k108k67k8.5k5.7k3.5k2.5k-
...
^{9}The GLUE data set distribution does not include the Test
...
tor S ∈R^{H} and an end vector E ∈R^{H} during
...
all of the words in the paragraph: Pi =�^{e}_{j}^{S}_{e}^{·}_{S}^{Ti}_{·Tj} .
...
^{11}QANet is described in Yu et al. (2018), but the system
...
^{12}The TriviaQA data we used consists of paragraphs from
...
<!-- page:8 -->
Dev Set
TasksMNLI-m QNLI MRPC SST-2 SQuAD
(Acc)(Acc)(Acc)(Acc)(F1)

BERTBASE84.488.486.792.788.5
...
(Al-Rfou et al., 2018). By contrast, BERT_{BASE}
...
676835.2480.682.290.7
6768124.6881.984.891.3
12768123.9984.486.792.9
12 1024163.5485.786.993.3
24 1024163.2386.687.893.7
...
Concat Last Four Hidden96.1-
Weighted Sum All 12 Layers95.5-
...
Results are presented in Table 7. BERT_{LARGE}
...
1. Z. Chen, H. Zhang, X. Zhang, and L. Zhao. 2018.
...
AAdditional Details for BERT

A.1Illustration of the Pre-training Tasks

We provide examples of the pre-training tasks in
the following.

Masked LM and the Masking ProcedureAs-
suming the unlabeled sentence ismy dog is
...
total).^{13} Training of BERT_{LARGE} was performed
...
Next Sentence PredictionThe next sentence
...
^{13}https://cloudplatform.googleblog.com/2018/06/Cloud-
...
A.4Comparison of BERT, ELMo ,and
OpenAI GPT
...
A.5Illustrations of Fine-tuning on Different
Tasks
...
BDetailed Experimental Setup

B.1Detailed Descriptions for the GLUE
Benchmark Experiments.

OurGLUEresultsinTable1areobtained
fromhttps://gluebenchmark.com/
...
_{[CLS]}^{Tok}[[CCLLSS]]TTookk  11 Tok 2... Tok N
...
SST-2The Stanford Sentiment Treebank is a
...
^{14}Note that we only report single-task fine-tuning results
...
C.1Effect of Number of Training Steps
...
1. 1. Question:Does BERT really need such
...
2. 2. Question: Does MLM pre-training converge
...
C.2Ablation for Different Masking
Procedures
